...
void loop(void)
{
  ...
  Serial.println("entering looped loop");
  while(1){
    ...
    if (newState == XDAScroller){  //enter here when we enter XDAScroller Mode
      Serial.println("XDASCROLLER MODE");
      locked=0;
      colorshifter();
    ...
    case XDAScroller: 
      {
        if (showClock==true)newState=AdkClock; //switch between XDAScroller and Clock mode
        L.ledWrite(9, 0, 0, 0);   //turn off colons
        L.ledWrite(2, 0, 0, 0);


       
      /* code from XDAScroller Main Loop here*/
        ...
        for(int i=0;i<6;i++){
          L.ledDrawLetter(i,newString.charAt(i),colorshift[stringTrim+i][0],colorshift[stringTrim+i][1],colorshift[stringTrim+i][2]);
        }
        delay(300);      
        break;
      }
    ...
    case AdkClock:
      {
        if (showClock==false)newState=XDAScroller;     
        uint16_t year;
    ...
    case AdkDisplay:
      {
        ...
        static uint8_t vals[NUM_LEDS][3] = {
          {
            0,                    }
          ,                };
        static const uint8_t doNotTouch[] = {
          49, 33, 17, 1, 48, 32, 16, 0                };
        ...
        if(L.getUptime() > nextTime){
          ...
          if(!nextTime){
            ...
            default: 
              settings.displayMode = AdkShowAnimation; //fallthrough
            ...
            case AdkShowColor:
              ...
              for(i = 0; i < 6; i += 2){
                ...
                val *= 100;
                val >>= isSigned ? 15 : 16;
                L.ledDrawLetter(i + 0, + val / 10 + '0', isSigned ? (j ? settings.bri : 0) : 
                cR, isSigned ? 0 : cG, isSigned ? (j ? 0 : settings.bri) : 
                cB);
                L.ledDrawLetter(i + 1, + val % 10 + '0', isSigned ? (j ? settings.bri : 0) : 
                cR, isSigned ? 0 : cG, isSigned ? (j ? 0 : settings.bri) : 
                cB);
              }
              break;
    ...
    case BTN_CLOCK:      //switch between XDAScroller and Clock
      Serial.println("button pressed");

      if (showClock==false){
        showClock=true;
        Serial.println("showClock true switching to  clock");
        newState = AdkClock;      
      } 
      else {
        showClock=false;
        Serial.println("showClock false switching to scroller");
        newState = XDAScroller;
        colorshifter();
      }
      break;
    ...
    case BTN_ALARM: 
      ...
      break;
    case BTN_BRIGHTNESS: 
      newState = AdkBrightness; 
      briSlider = L.capSenseSlider(); 
      break;
    case BTN_COLOR:      
      newState = AdkColor;      
      colorSlider = L.capSenseSlider(); 
      break;
    case BTN_PRESETS:    
      newState = AdkPresets;    
      break;
    case BTN_DISPLAY:    
      newState = AdkDisplay;    
      nextTime = 0; 
      speedSlider = L.capSenseSlider(); 
      break;
    case BTN_VOLUME:     
      newState = AdkVolume;     
      volSlider = L.capSenseSlider(); 
      break;
    }
    if(newState != AdkInvalid){
      writeSettings();
...
#define BT_CMD_XDASCROLLER                 17    // () -> (u8) +  *  -> (M,E,S,S,A,G,E): chars
...
static uint16_t adkProcessCommand(uint8_t cmd, const uint8_t* dataIn, uint16_t sz, char fromBT, uint8_t* reply, uint16_t maxReplySz){  //returns num bytes to reply with (or 0 for no reply)
  ...
  case BT_CMD_XDASCROLLER:
    {
      //set up scroller variables here  
      Serial.println("Received scroller command");
      showClock=false;
      message="";
      Serial.print("Received Data: ");
      for (int counter1 = 0; counter1 < sz; counter1++) {

        char c= char(dataIn[counter1]);
        message=message+c;
        Serial.print (c); 
      }
      Serial.println("");
      message="      "+message;
      Serial.println("set clock to false;");
    }
    break;
